...
Agama.prototype._clearAgama = function() {
  this._constructionelements.splice(0, this._constructionelements.length);
  this._patternlines.splice(0, this._patternlines.length);
...
Agama.el.getActualParameters = function() {
  ...
  } else if (this.agamatype === 'point') {
    return {cx: this.actualAttr('cx'),
            cy: this.actualAttr('cy'),
            r: this.actualAttr('r'),
            id: this.agamapointid,
            toString: function() {
              return 'x:' + this.cx.toFixed(1) +' y:' + this.cy.toFixed(1) +
                ' id: ' + this.id;
            }
           };
  } else if (this.agamatype === 'line') {
...
Agama.prototype.point = function(a, c) {
  ...
  k.agamatype = 'point';
  k.setOpacity();
  k.agamapointid = this._nextpointid++;
  this.addToPoints(k);
  return k;
};
