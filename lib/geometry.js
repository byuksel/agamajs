 ...
 * @return {Array<Object>} - intersection points, defined as an object
*                            with 'x', 'y' and 'agamatype'.
 */
Geometry.prototype.getIntersectionCircleCircle = function(x1, y1, r1,
                                                          x2, y2, r2) {
  // Should throw an error for inadequate number of arguments as otherwise
  // this method will find incorrect results.
  if (arguments.length !== 6) {
    throw new Error('Need total of 6 values as an argument');
  }
  ...
  var thirdMul = 0.5 * Math.sqrt((2 * r1r2plus / bigRSq) - (
    r1r2minus * r1r2minus / (bigRSq * bigRSq)) - 1);

  var interX1 = firstMul * (x1 + x2) +
        secondMul * (x2 - x1) + thirdMul * (y2 - y1);
  var interY1 = firstMul * (y1 + y2) +
        secondMul * (y2 - y1) + thirdMul * (x1 - x2);

  var interX2 = firstMul * (x1 + x2) +
        secondMul * (x2 - x1) - thirdMul * (y2 - y1);
  var interY2 = firstMul * (y1 + y2) +
        secondMul * (y2 - y1) - thirdMul * (x1 - x2);

  var returnVal =  [];
  if (!isNaN(interX1) && !isNaN(interY1)) {
    returnVal.push({x: interX1, y: interY1, agamatype: 'pivot'});
  }
  if ((!isNaN(interX2) && !isNaN(interY2)) &&
      (interX1 !== interX2 || interY1 !== interY2)) {
    returnVal.push({x: interX2, y: interY2, agamatype: 'pivot'});
  }
  return returnVal;
};

/**
 * Finds intersection points of one circle and one line segment.
 ...
 * @param {number} linex2 - X coordinate of second end point on the line segment.
 * @param {number} liney2 - Y coordinate of second end point on the line segment.
...
Geometry.prototype.getIntersectionLineSegmentCircle = function(
  ...
  if (arguments.length !== 7) {
    throw new Error('Need total of 7 values as an argument');
  }
  var dx = linex2 - linex1;
  var dy = liney2 - liney1;
  var preResult = [];
  if (this._isFloatEqual(dx, 0) && this._isFloatEqual(dy, 0)) {
    return preResult;
  }
  ...
  if (this._isFloatLessEqual(t1, 1)  && this._isFloatGreaterEqual(t1, 0)) {
    preResult.push({x: linex1 + t1 * dx, y: liney1 + t1 * dy,
                    agamatype: 'pivot'});
  }
  var t2 = t + dt;
  if (this._isFloatLessEqual(t2, 1)  && this._isFloatGreaterEqual(t2, 0)) {
    preResult.push({x: linex1 + t2 * dx, y: liney1 + t2 * dy,
                    agamatype: 'pivot'});
  }
...
*/
Geometry.prototype.getIntersectionLineSegmentLineSegment = function(
  ax1, ay1, ax2, ay2, bx1, by1, bx2, by2) {
  // Should throw an error for inadequate number of arguments as otherwise
  // this method will find incorrect results.
  if (arguments.length !== 8) {
    throw new Error('Need total of 8 values as an argument');
  }
  var m1 = ay2 - ay1;
  var m2 = ax1 - ax2;
 ...
 */
Geometry.prototype.getIntersectionPointLineSegment = function(
  px, py, x1, y1, x2, y2) {
  if (this._isFloatEqual(this.getDist(x1, y1, x2, y2),
                         this.getDist(px,py,x1,y1) +
                         this.getDist(px,py,x2,y2))) {
    return [{x: px, y: py}];
  }
  return [];
};
 ...
 *
 * It is also true if their difference is less than @see this.EPSILON.
 *
...
Geometry.prototype._isFloatGreaterEqual = function(x, y) {
  return this._isFloatEqual(x, y) || x > y;
};
